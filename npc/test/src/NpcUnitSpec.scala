...
class NpcUnitSpec extends AnyFlatSpec {
  ...
  it should "write CSRs and capture trap state" in {
    simulate(new Csr(32)) { dut =>
      ...
      dut.io.addr.poke(CsrAddr.Mtvec)
      dut.io.cmd.poke(CsrCmd.None)
      dut.io.src.poke(0.U)
      dut.io.wen.poke(false.B)
      dut.io.trapReq.valid.poke(false.B)
      dut.io.trapReq.pc.poke(0.U)
      dut.io.trapReq.cause.poke(0.U)
      dut.clock.step()
      dut.reset.poke(false.B)

      dut.io.cmd.poke(CsrCmd.Write)
      dut.io.src.poke("h80000100".U)
      dut.io.wen.poke(true.B)
      dut.clock.step()
      dut.io.wen.poke(false.B)
      dut.io.rdata.expect("h80000100".U)

      dut.io.trapReq.valid.poke(true.B)
      dut.io.trapReq.pc.poke("h80000020".U)
      dut.io.trapReq.cause.poke(2.U)
      dut.io.trapVector.expect("h80000100".U)
      dut.clock.step()
      dut.io.trapReq.valid.poke(false.B)

      dut.io.addr.poke(CsrAddr.Mepc)
      dut.io.rdata.expect("h80000020".U)
      dut.io.epc.expect("h80000020".U)

      dut.io.addr.poke(CsrAddr.Mcause)
      dut.io.rdata.expect(2.U)
    }
  }
}
