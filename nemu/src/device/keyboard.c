...
#define NEMU_KEYS(f)                                                              \
        f(ESCAPE) f(F1) f(F2) f(F3) f(F4) f(F5) f(F6) f(F7) f(F8) f(F9) f(        \
                ...
                f(8) f(9) f(0) f(MINUS) f(EQUALS) f(BACKSPACE) f(TAB) f(Q) f(     \
                        ...
                        f(RIGHTBRACKET) f(BACKSLASH) f(CAPSLOCK) f(A) f(S) f(     \
                                D) f(F) f(G) f(H) f(J) f(K) f(L) f(SEMICOLON)     \
                                f(APOSTROPHE) f(RETURN) f(LSHIFT) f(Z) f(X) f(    \
                                        C) f(V) f(B) f(N) f(M) f(COMMA)           \
                                        f(PERIOD) f(SLASH) f(RSHIFT) f(LCTRL) f(  \
                                                APPLICATION) f(LALT) f(SPACE)     \
                                                f(RALT) f(RCTRL) f(UP) f(         \
                                                        DOWN) f(LEFT) f(RIGHT)    \
                                                        f(INSERT) f(DELETE) f(    \
                                                                HOME) f(END)      \
                                                                f(PAGEUP) f(      \
