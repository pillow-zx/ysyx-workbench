...
static void decode_rm(Decode *s, int *rm_reg, word_t *rm_addr, int *reg,
                      int width)
{
        ModR_M m;
...
#define INSTPAT_MATCH(s, name, type, width, ... /* execute body */)            \
        {                                                                      \
                ...
                decode_operand(s, opcode, &rd, &src1, &addr, &rs, &gp_idx,     \
                               &imm, w, concat(TYPE_, type));                  \
                s->dnpc = s->snpc;                                             \
                __VA_ARGS__;                                                   \
        }

static void decode_operand(Decode *s, uint8_t opcode, int *rd_, word_t *src1,
                           word_t *addr, int *rs, int *gp_idx, word_t *imm,
                           int w, int type)
{
        switch (type) {
        case TYPE_I2r:
...
again:
        ...
        INSTPAT("0000 1111", 2byte_esc, N, 0,
                _2byte_esc(s, is_operand_size_16));

        INSTPAT("0110 0110", data_size, N, 0, is_operand_size_16 = true;
                goto again;);
